...
import (
	...
	"github.com/williamsharkey/compilePub/gotool/web"
...
func RunGet(cmd *base.Command, downloadOnly, update, force, testDeps bool, args []string, cwd string) {
	//work.AddBuildFlags(cmd, cwd)
	//var getD = cmd.Flag.Bool("d", false, "")
	//var getF = cmd.Flag.Bool("f", false, "")
	//var getT = cmd.Flag.Bool("t", false, "")
	//var getU = cmd.Flag.Bool("u", false, "")
	...
	for _, arg := range args {
		download(cmd, arg, nil, &stk, mode, cwd)
	}
	base.ExitIfErrors()
...
func download(cmd *base.Command, arg string, parent *load.Package, stk *load.ImportStack, mode int, cwd string) {
...
func downloadPackage(cmd *base.Command, p *load.Package) error {
	var (
		vcs            *vcsCmd

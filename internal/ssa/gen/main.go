...
func genOp() {
	...
	fmt.Fprintln(w, "\"github.com/williamsharkey/compilePub/internal/obj\"")
	for _, a := range archs {
		if a.pkg != "" {
			fmt.Fprintf(w, "%q\n", a.pkg)
		}
	...
	for _, a := range archs {
		...
		for i, r := range a.regnames {
			pkg := a.pkg[len("github.com/williamsharkey/compilePub/internal/obj/"):]
			var objname string // name in github.com/williamsharkey/compilePub/internal/obj/$ARCH
			switch r {
			case "SB":
				// SB isn't a real register.  github.com/williamsharkey/compilePub/internal/obj expects 0 in this case.
				objname = "0"
			case "SP":
				objname = pkg + ".REGSP"
			case "g":
